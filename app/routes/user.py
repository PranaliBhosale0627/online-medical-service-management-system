...
def cancel_order(order_id):
    order = Order.query.get_or_404(order_id)
    # Allow cancellation for Pending, Confirmed, and Order Confirmed statuses
    allowed_statuses = ['Pending', 'Confirmed', 'Order Confirmed']
    
    if order.user_id != current_user.id or order.status not in allowed_statuses:
    ...
    try:
        ...
        # If paying online, deduct from admin wallet (as it was added during checkout)
        if order.payment_method in ['UPI', 'Card']:
            admin = User.query.filter_by(role='admin').first()
            if admin and admin.wallet_balance is not None:
                admin.wallet_balance -= order.total_amount
                # Optionally add to user wallet if you want to implement a refund system
                current_user.wallet_balance += order.total_amount

        order.status = 'Cancelled'
        db.session.commit()
        flash('Order cancelled successfully. Stock restored.', 'success')
    except Exception as e:
        db.session.rollback()
